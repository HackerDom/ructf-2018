...
for num in {1..32}; do
    ...
    if ! iptables -t nat -C PREROUTING -i eth0.$((num+100)) -p tcp -m tcp -m comment --comment closednetwork -j DNAT --to-destination ${ip}:40002 &> /dev/null; then
        iptables -t nat -A PREROUTING -i eth0.$((num+100)) -p tcp -m tcp -m comment --comment closednetwork -j DNAT --to-destination ${ip}:40002 &> /dev/null
