...
for num in {1..32}; do
    ...
    while iptables -t nat -C PREROUTING -i eth0.$((num+100)) -d 10.0.0.0/8 -p tcp -m tcp -m comment --comment closednetwork -j DNAT --to-destination ${ip}:40002 &>/dev/null; do
        iptables -t nat -D PREROUTING -i eth0.$((num+100)) -d 10.0.0.0/8 -p tcp -m tcp -m comment --comment closednetwork -j DNAT --to-destination ${ip}:40002
    done;
done
